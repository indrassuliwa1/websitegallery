    ...
    <div className="text-white min-h-screen font-sans selection:bg-purple-500 overflow-x-hidden">
            ...
            <nav className="fixed top-0 left-0 w-full z-40 p-4">
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 mx-auto max-w-lg px-5 py-3 rounded-2xl flex justify-between items-center shadow-2xl">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-gradient-to-tr from-blue-600 to-purple-600 rounded-lg flex items-center justify-center font-bold text-[10px]">
                    RBX
                  </div>
                  <span className="font-bold text-xs italic uppercase tracking-widest">
                    Indra Gallery
                  </span>
                </div>
                <button
            ...
            <main className="max-w-5xl mx-auto px-4">
              ...
              <section className="pt-32 pb-20 text-center">
                <header className="mb-12">
                  <h2 className="text-4xl font-black uppercase tracking-tighter drop-shadow-lg">
                    Gallery Rosblok 🧊
                  </h2>
                  <p className="text-blue-100 text-sm opacity-60 italic">
                    "Melihat kegabutan saya dalam meng-scinsut roblog"
                  </p>
                </header>
                  ...
                  {robloxPhotos.map((photo, index) => (
                    ...
                    >
                      ...
                      <div className="mt-3 flex justify-between items-center px-1">
                        ...
                        <Heart
                          size={14}
                          className="text-pink-500 fill-pink-500 group-hover:scale-125 transition-transform"
                        />
                      </div>
                    </motion.div>
                  ))}
              ...
              <section
                ref={videoSectionRef}
                className="py-40 min-h-screen border-t border-white/5"
              >
                <div className="text-center mb-16">
                  <div
                    className={`inline-flex items-center gap-2 px-4 py-1 rounded-full text-xs font-bold mb-4 animate-bounce transition-colors duration-500 ${
                      isVideoSection ? "bg-purple-600" : "bg-blue-600"
                    }`}
                  >
                    <Film size={14} /> NEW CLIPS
                  </div>
                  <h2 className="text-5xl font-black uppercase italic tracking-tighter drop-shadow-lg">
                    Epic Moments! 🎬
                  </h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  {robloxVideos.map((video) => (
                    ...
                    >
                      ...
                      <div
                        className={`p-6 transition-colors duration-1000 ${
                          isVideoSection ? "bg-purple-900/20" : "bg-blue-900/20"
                        }`}
                      >
              ...
              <section ref={guestbookRef} className="mt-40 mb-32 px-4 relative">
                ...
                >
                  <div className="text-center mb-10">
                    <h2 className="text-3xl font-black uppercase italic tracking-tighter mb-2 text-[#EEA727]">
                      Buku Tamu ✍️
                    </h2>
                    <p className="text-[10px] opacity-50 uppercase tracking-[0.3em] text-white">
                      Tinggalkan pesan di bawah!
                    </p>
                  </div>
                ...
                {/* --- SECTION: BUKU TAMU TERBARU (RESPONSIVE MARQUEE) --- */}
                <div className="mt-8 md:mt-12 relative overflow-hidden w-full py-6 md:py-10">
                  <div className="absolute inset-y-0 left-0 w-16 md:w-32 bg-gradient-to-r from-transparent to-transparent z-10 pointer-events-none" />
                  <div className="absolute inset-y-0 right-0 w-16 md:w-32 bg-gradient-to-l from-transparent to-transparent z-10 pointer-events-none" />
                  ...
                  >
                    {[
                      ...
                      <motion.div
                        ...
                        // min-w diatur 220px untuk HP, 300px untuk Laptop
                        className="bg-white/10 backdrop-blur-xl border border-white/20 p-4 md:p-6 rounded-2xl md:rounded-3xl shadow-2xl min-w-[220px] md:min-w-[300px] max-w-[240px] md:max-w-[320px] relative overflow-hidden group"
                        style={{
                          boxShadow:
                            "0 15px 35px -10px rgba(238, 167, 39, 0.2)",
                        }}
                      >
                        <div className="absolute top-0 left-0 w-1 md:w-1.5 h-full bg-[#EEA727] opacity-60" />

                        <div className="flex justify-between items-start mb-2 md:mb-3">
                          <div className="flex flex-col">
                            <span className="text-[10px] md:text-[13px] font-black text-[#EEA727] uppercase tracking-wider">
                              @{item.name}
                            </span>
                            <span className="text-[8px] md:text-[10px] opacity-40 italic text-white">
                              {item.created_at
                          ...
                          <Heart
                            size={14}
                            className="text-[#EEA727] opacity-20"
                          />
                        </div>

                        {/* Font size di HP (text-xs), di Laptop (text-sm) */}
                        <p className="text-xs md:text-sm opacity-90 leading-relaxed font-medium text-white italic line-clamp-4">
                          "{item.message}"
                        </p>
                  ...
                  {/* --- GIF DEKORASI POJOK KANAN BAWAH (RESPONSIVE) --- */}
                 <div className="fixed bottom-4 right-4 z-[50] pointer-events-none">
                    <img
                      src="/IMAGES/output-onlinegiftools (3).gif"
                      alt="decoration"
                      className="w-20 h-auto md:w-32 opacity-80"
                    />
                  </div>
                </div>
              </section>
      ...
      <AnimatePresence>
        {selectedImg && (
          ...
          >
            <motion.button className="absolute top-6 right-6 p-3 bg-white/10 rounded-full hover:bg-white/20">
              <X size={24} />
            </motion.button>
            <motion.img
